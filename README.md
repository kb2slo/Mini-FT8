...
- **Install / update the browser app (`http://minift8.local/update`).** After pairing, the page fetches the bundle from GitHub Pages (`…/Mini-FT8/app`) over HTTPS and writes it onto the sidekick. Human landing for the same site: `https://kb2slo.github.io/Mini-FT8/`. **Restore seed pages** (same page, or status) rewrites provision/status/update/log from firmware without wiping an installed app.
