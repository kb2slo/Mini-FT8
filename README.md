...
**USB Drive (`C`):** expose internal FATFS as a USB disk. Best on Mac or PC; iPhone Files is unreliable. Unplug the radio from USB-C first; the same port cannot be USB host and a disk at once. Press `C`. The Cardputer enumerates as `USB DISK`. The screen says `Waiting for computer...` until the computer actually attaches. On Mac or Windows, copy logs off the disk, eject it in Finder/Explorer, then press `C` again to return to RX. If the screen says `USB busy, unplug radio`, disconnect the radio's USB cable first. iPhone/iPad Files is best-effort only.
