...
- **Busy band, busy UI.** Opening QSO browse or copying files does not stall the slot loop. Low battery abort is visible on R.
- **A beacon that keeps beaconing.** Beacon CQ re-arms after TX and after empty receive slots, so a quiet band no longer stops it. Previously the CQ was only re-queued at the tail of a successful decode, so with nothing to decode the beacon went silent and the TX queue stayed empty — on a dead band it would simply stop and never restart. Replies still take priority over the beacon, and turning beacon OFF drops the queued CQ immediately instead of letting one more fire. Confirmed 2026-09-08 by a 3.6-hour unattended run: 544 transmit slots spotted by 1,195 receivers, no gap over five minutes. **There is no time limit yet** (ROADMAP B37) — an unattended beacon now runs until you stop it.
- **KH1 support dropped, for now.** QMX and QDX only. `radio_control_kh1_*`, KH1-MIC's mic-capture path, and the PORTA-sharing policy they needed are gone (I21) — I don't own a KH1, so that path was never field-verified, and it would only have gotten harder to carry blind through the PORTA/UART refactor now underway for the Nano companion (I3). Open to bringing it back if someone with a KH1 wants to pick it up and verify against real hardware.
...
**Sync iPhone (`H` then `1`):** unplug the radio from Cardputer USB-C first. Then start one-shot BLE time sync. On the iPhone: nRF Connect or LightBlue → connect to `Mini-FT8-<call>` and Pair if asked. Use nRF Connect again after a reboot; the Settings Bluetooth list is not this path. Wait for **Time OK**, then plug the radio in and **S → 2**. STATUS time shows `P` after a successful read and is UTC, not the iPhone lock-screen clock. This does not set grid. If the screen says **No 2A0F**, the phone did not expose timezone and time is not applied. If the radio stays on USB-C during sync, CAT/`TA` often dies afterward.

**USB Drive (`C`) — removed in this fork.** `C` is unassigned; the key does nothing. It never actually worked: the Cardputer does not enumerate as a USB disk at all, because the USB host stack is brought up at boot and leaves the OTG core in host mode, so the disk gadget is never visible to the computer (the screen would sit on `Waiting for computer...` forever). See [RFC 0001](docs/rfcs/0001-ble-companion.md) §5.5 and ROADMAP B23. **Get logs off with Copy Files to SD** (`O` then the copy entry), which is unchanged, or read them on-device with the QSO browser (`Q`). Wei's key table and operation manual below still list `C`; the fork notes here take precedence.

**USB Serial/JTAG host commands (`WRITEBIN` / `READ` / `LIST` / `DELETE`) — removed in this fork.** The Cardputer no longer answers a serial command protocol on its own USB-C, and `tools/pc_terminal.py` / `tools/writebin.py` are gone with it. **It had already been non-functional since V2.0.2** (2026-05-02): the call that polled the port was dropped upstream on 2026-04-30 when MSC log export replaced CONTROL mode, leaving the code compiled but unreachable behind `[[maybe_unused]]`. V2.0.1 is the last release in which it worked; every binary since — including `release/MiniFT8_V2.1b.bin` and `V2.1d.bin` shipped here — silently ignores those commands. B23 then removed MSC too, so nothing succeeded it. **Get logs off with Copy Files to SD** (`O` then the copy entry), or read them on-device with the QSO browser (`Q`). Wei's `GNSS_LoRa` note below still says "USB Serial/JTAG host commands still work"; the fork notes here take precedence.

- **Browser clock and free-text TX (`http://minift8.local/log`).** With the AtomS3 Lite sidekick on Port A and WiFi provisioned, the page shows live host log and decodes without a token. Pair once (press the sidekick's user button). Clock sync and the TX / Cancel controls need that token — they are writes. Free-text TX is the same one-shot path as MENU `Send FreeText`.
- **Install / update the browser app (`http://minift8.local/update`).** After pairing, the page fetches the bundle from GitHub Pages (`…/Mini-FT8/app`) over HTTPS and writes it onto the sidekick; install opens **`/app.html`** (station setup, Connect / Tune, then QSO controls). Human landing for the same site: `https://kb2slo.github.io/Mini-FT8/`. **Restore seed pages** (same page, or status) rewrites provision/status/update/log from firmware without wiping an installed app.
...
**Radio picker (`N` → `3`):** `KH1-USBC` / `KH1-MIC` are gone from this fork — QMX and QDX only (I21). The manual below, including the KH1 Connections section and its mode-map row, is Wei's original and no longer applies here.
