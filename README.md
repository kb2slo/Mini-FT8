...
- **Sync time from an iPhone.** `H` then `1`: Cardputer advertises `Mini-FT8-<call>`. Connect with nRF Connect or LightBlue every time (Settings → My Devices will connect and drop without setting time). Time only, no grid. NimBLE is torn down after the read.
...
**Sync iPhone (`H` then `1`):** start one-shot BLE time sync. On the iPhone: nRF Connect or LightBlue → connect to `Mini-FT8-<call>` and Pair if asked. Use nRF Connect again after a reboot; the Settings Bluetooth list is not this path. STATUS time shows `P` after a successful read. This does not set grid.
