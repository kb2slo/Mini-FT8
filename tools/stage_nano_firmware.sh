#!/usr/bin/env bash
# Copies a built nano_companion image into components/nano_flasher/target_firmware/
...
NANO_BUILD="$ROOT/nano_companion/build"
DEST="$ROOT/components/nano_flasher/target_firmware"

if [[ ! -f "$NANO_BUILD/nano_companion.bin" ]]; then
    echo "error: $NANO_BUILD/nano_companion.bin not found." >&2
    echo "Build it first: (cd nano_companion && idf.py build)" >&2
    exit 1
fi
...
cp "$NANO_BUILD/nano_companion.bin" "$DEST/nano_companion.bin"
