...
// freq_len(1) | freq... | my_grid_len(1) | my_grid... | comment_len(1) | comment...
                                         ...
                                         uint8_t *out, size_t out_cap)
{
    ...
    p += freq_n;
    payload[p++] = (uint8_t)my_grid_n;
    memcpy(&payload[p], e->my_grid, my_grid_n);
    p += my_grid_n;
...
bool porta_proto_parse_file_entry_row(const porta_frame_t *f, porta_qso_entry_row_t *out)
{
    ...
    if (freq_n > PORTA_FREQ_MAX || p + freq_n + 1 + 1 > f->len) {
        return false;
    }
    ...
    const uint8_t my_grid_n = f->payload[p++];
    if (my_grid_n > PORTA_GRID_MAX || p + my_grid_n + 1 > f->len) {
        return false;
    }
    memcpy(out->my_grid, &f->payload[p], my_grid_n);
    out->my_grid[my_grid_n] = '\0';
    p += my_grid_n;
