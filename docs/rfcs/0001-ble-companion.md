...
What **is** achievable without any new wiring: the M5NanoC6 has exactly one on-board button, wired to GPIO9 (M5Stack's own docs: "hold down the GPIO9 button and then connect the data cable" for desk bring-up). `sidekick`, already running and powered via PORTA, can read that same GPIO9 pin as a plain input during normal operation (nothing else claims it today) and self-call `esp_restart()` on a sustained press — the operator's finger is still physically holding GPIO9 low at that exact reset, so the chip lands in ROM download mode exactly as if a cable had just been plugged in fresh. Everything after that point — connect, compare version, write the image — can be fully automated over PORTA using `esp-serial-flasher`'s stock UART transport, the same `esp_loader_*` calls the USB-C path already uses, just a different transport underneath. Semi-manual (one deliberate button hold), not silent, but needs zero hardware changes. Not built yet; this is the plan for when this slice is picked up.

**Future phase, not scheduled:** a dedicated 5th wire between ADV and Nano carrying GPIO9 (or GPIO9+EN, mirroring the DTR/RTS pair esptool itself uses over USB) would let the ADV trigger download mode directly, without the button-hold — fully silent, hands-off updates over PORTA. Deliberately not pursued now: it means leaving a clean Grove-to-Grove cable for a custom connector/wiring, a real physical complication to the enclosure and cabling, only worth it if the button-hold's one manual step turns out to be real friction in practice.
