...
**Field flash over USB-C (pivot, §5.1):** a factory (unflashed) Nano on ADV USB-C is still recognized by B18 presence (`303A:*`). The install prompt (§5.4) drives the ADV, acting as USB host, through the Nano's ROM serial bootloader protocol (sync + flash-begin/flash-data, the same handshake `esptool.py` uses) to write the embedded `.bin`. This needs the Nano's native USB Serial/JTAG peripheral to support the same auto-reset-into-bootloader handshake `esptool.py` uses from a PC — **not yet proven with the ADV as the host side**; if it does not work reliably, the fallback is desk flash on the Nano's own USB-C (unchanged from the original plan) and the field-flash prompt is dropped or gated behind a "connect via computer instead" message. Desk flash always remains available regardless of field-flash outcome.
