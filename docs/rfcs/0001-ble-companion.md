...
**Field flash over USB-C (pivot, §5.1):** a factory (unflashed) Nano on ADV USB-C is still recognized by B18 presence (`303A:*`). The install prompt (§5.4) drives the ADV, acting as USB host, through the Nano's ROM serial bootloader protocol (sync + flash-begin/flash-data, the same handshake `esptool.py` uses) to write the embedded `.bin`. This needs the Nano's native USB Serial/JTAG peripheral to support the same auto-reset-into-bootloader handshake `esptool.py` uses from a PC. `components/nano_flasher/` (2026-09-03) builds and links this correctly — `esp_loader_*` + `esp32_usb_cdc_acm_port` wired to a real Green Nano attach event, embedded `nano_companion` bootloader/partition-table/app confirmed byte-exact in the shipped ADV image via `nm`. **Proven on real hardware (2026-09-03):** ADV field-flashed a genuine factory M5NanoC6 over its own USB-C host link; `idf.py monitor` on the Nano's own USB-C afterward showed it booting `nano_companion` (not the stock `nanoc6_factorytest` firmware it shipped with). First attempt failed silently (a host-lifecycle bug — `cdc_acm_host_install()` was called after a full USB host teardown with nothing to attach to); fixed by explicitly reinstalling the host and calling `usb_c_presence_yield_device()` before handing off to the flasher, same as UAC already does for QMX. `tools/restore_nano_stock.sh` (full 4 MB flash backup/restore) re-greened the same physical part for a clean retest, which passed. Fallback (desk flash on the Nano's own USB-C) remains available and unchanged if a future unit doesn't cooperate.

**Two flash mechanisms, not two configurations of one** (2026-09-03 design decision): a **green/factory Nano has no code that can answer a software download-mode request**, so its *only* entry point is the native-USB ROM bootloader — USB-C is mandatory for the first flash, not just preferred. Once a Nano is running companion firmware, that firmware can implement its own RTC-scratch "reboot into UART download mode" handler (a standard ESP32 technique, no external strapping needed), making **later updates reachable over PORTA Grove UART** without USB-C at all. So: **seed flash** = ROM bootloader over USB-C CDC (`esp-serial-flasher` + a new custom USB-CDC-host transport — the library ships UART/SPI transports, not this). **Update flash** (later slice, not this phase) = companion firmware's own download-mode hook over PORTA Grove UART, possibly still `esp-serial-flasher`'s stock UART transport once the Nano is in that state. Use `esp-serial-flasher` (Espressif's own chip-flashes-chip library, already in the component-registry pattern this tree uses for `espressif/usb_host_cdc_acm`) rather than hand-rolling the ROM protocol.
...
| **ROM-bootloader auto-reset over USB-CDC doesn't work from ADV** | **Resolved.** Proven on real hardware 2026-09-03 (`components/nano_flasher/`): field-flashed a factory M5NanoC6, confirmed booting `nano_companion` via monitor on its own USB-C afterward. Fallback (desk flash, §5.2) stays available regardless. |
