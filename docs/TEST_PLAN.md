...
| From | Check | Status |
| --- | --- | --- |
| I28c (web LittleFS partition) | After flashing a sidekick build that includes this table: `cd sidekick && idf.py build flash`, then `idf.py partition-table` (or the flash log) shows a `web` / `littlefs` row at `0x620000` / `0x1E0000`, and the device still boots to the existing provision/status/`/log` behaviour (still served from firmware embed until hydrate lands). `nvs` WiFi + pairing token must still be present after the reflash only if this flash preserved NVS — a full `idf.py flash` may wipe it; if so, re-provision once and confirm token mint. | **owed** — partition CSV only; no hydrate/POST yet |
