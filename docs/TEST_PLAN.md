...
| I28d (beacon / queue_cancel / queue_reply dispatch) | `main.cpp`/`porta.cpp` ACTION handlers for `PORTA_ACT_BEACON`/`QUEUE_CANCEL`/`QUEUE_REPLY` (RFC 0004 §11) — `idf.py build` clean on both ADV and sidekick, but nothing exercises them yet: there is no sidekick HTTP relay for these three ACTIONs, so a live Port A round-trip needs that relay first (or raw frame injection, which is not a realistic field check). On hardware once the relay exists: BEACON with an out-of-range mode NAKs `unknown beacon mode`; a valid mode applied while `board_power_halted()` (non-OFF) NAKs `battery halt`; a valid mode cycles the same CQ-enqueue/cancel side effects as STATUS key 1, and the STATUS screen (if open) reflects it live. QUEUE_CANCEL with an id not in the active zone NAKs `unknown entry`; a real id drops that entry regardless of its current queue position. QUEUE_REPLY with a decode_id that has scrolled out of the 64-entry ring NAKs `unknown decode`; a live one behaves exactly like an R-screen tap (arms TX unless one is in flight, queues behind a live head, refuses/ignores per the same rules `host_test_unique_callsign` already pins). | **not built yet** — blocked on the sidekick relay row above |
| I28d (FILE_LIST / FILE_READ dispatch) | `main.cpp`/`porta.cpp` async FS handlers (RFC 0004 §11) — `idf.py build` clean, same relay block as the row above. On hardware once the relay exists: FILE_LIST for a kind other than QSO-daily NAKs `unknown kind`; a valid request returns day-file names newest-first, paged by skip/take, terminated by ACK — fewer rows than `take` means end of list. FILE_READ for a missing/bad filename NAKs `open failed`; a valid request returns parsed rows (time/band/call/rst, `-99` for no report) rather than raw ADIF, terminated by ACK the same way. A second FILE_LIST or FILE_READ request while one is still in flight replaces it (last-request-wins), and does not corrupt or stall the one already running from the on-device Q/Delete screens if either is open at the same time — this last point is the one worth an operator actively trying to provoke (open Q on-device, request FILE_LIST/FILE_READ from the bench at the same moment), since it is exactly the class of bug that would not show up from either path alone. | **not built yet** — blocked on the sidekick relay row above |
