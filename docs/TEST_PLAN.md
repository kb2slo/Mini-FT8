...
| I28d (app.html Operate/Log screens, end-to-end QSO) | New bottom-tab `app.html` (RFC 0004 §11: Operate default, Log, Settings) — no `idf.py build` to prove this, it is browser JS. Checked so far only in a JS-console simulation with fabricated events (no live ADV/sidekick): tab switching, countdown/parity computed from the phone's own clock, beacon label from `cq_type` + the latest `SLOT_STATE`, queue rows sorted by state and removed on `IDLE` (state 6), decode rows newest-first with a `→ ME` tag, `/api/log/files`+`/api/log/entries` error paths. Nothing here has touched real Port A traffic. On hardware: pair, watch the state header populate (band/freq/CAT/audio/offset) and the countdown actually count down in sync with real TX; tap Beacon and confirm a CQ goes out on the resolved offset; tap a decode and confirm it queues, progresses through the visible states, and clears; open Log and confirm the just-logged QSO appears under Today with the right band/RST. This is the operator's own end-to-end validation — do not report it as passed from the simulation above. | **owed** — full end-to-end QSO with a phone, ADV and sidekick together |
