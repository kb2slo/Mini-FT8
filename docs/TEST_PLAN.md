...
| `host_test_porta_proto` | Sidekick/main frame codec (I28a): encode/decode round-trip, zero-length and 255-byte payloads, and a sync byte appearing *inside* a payload — which it will, since payloads are binary. Sweeps **every single-bit flip** in every framed byte and requires the CRC to reject all of them, which a sum-of-bytes checksum (what the old beacon used) does not. Pins recovery after truncation and after leading garbage, and pins the one deliberate limitation: a corrupt frame is dropped without rescanning the bytes it consumed, so a stray sync byte in noise costs exactly one real frame. Counters survive the decoder's own resync but not an explicit init — a distinction that started as a bug, since preserving them meant reading uninitialised memory on first use. Also pins **TX free-text and cancel** ACTION verbs and **CONFIG_GET/SET/VALUE** key/value framing (`PORTA_ACT_TX_FREE` / `TX_CANCEL`): empty text refused at encode, overlong text truncated to `PORTA_EVENT_TEXT_MAX`, cancel is verb-only and not parseable as free-text |
