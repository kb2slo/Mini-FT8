...
| I7 | ci | Hash-first build identity | In progress on `feat/build-identity`. `a9f64c2-minift8-dev.bin` (dirty: `-dirty-`); PERF line `dev a9f64c2*`. CI artifact glob still on `ci` until that PR merges. |
